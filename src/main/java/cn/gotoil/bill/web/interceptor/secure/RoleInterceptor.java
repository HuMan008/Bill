...
public class RoleInterceptor implements HandlerInterceptor {
    ...
    private boolean userAuthValidate(HttpServletRequest request, Object handler) {
        HasRole hander = ((HandlerMethod) handler).getMethodAnnotation(HasRole.class);
        //如果没写权限要求，那就直接验证通过
        if (hander == null) {
            return true;
        }
        ...
        if (StringUtils.isNotEmpty(hander.value())) {
            return roles.contains(hander.value());
        }
        ...
        if (hander.values().length > 0) {
            //要求全匹配
            if (hander.needAll()) {
                Set<String> needRoles = Sets.newHashSet(hander.values());
